...
class AdvCIFAR10DSET(datasets.vision.VisionDataset):
    ...
    def __init__(self, root, augmentation_config_file, device, db_name='robust', transform=None):
        super().__init__(root)
        # self.db_root = PATH_DICT['advcifar10']
        self.folder_names = {'det': 'ddet_CIFAR', 'nonrobust': 'd_non_robust_CIFAR', 'rand': 'drand_CIFAR',
                             'robust': 'd_robust_CIFAR'}
        self.url = 'http://andrewilyas.com/datasets.tar'
        self.rar_file = "datasets.tar"
        self.data_file = 'CIFAR_ims'
        self.lab_file = 'CIFAR_lab'
        ...
        # self.db_folder = os.path.join(self.root, self.folder_names[db_name])
        ...
        self.data = torch.cat(torch.load(os.path.join(self.root, self.folder_names[db_name], self.data_file)))
        self.targets = torch.cat(
            torch.load(os.path.join(self.root, self.folder_names[db_name], self.lab_file)))

        self.mean = self.data.mean(axis=(0, 2, 3)).to(device)
        self.std = self.data.std(axis=(0, 2, 3)).to(device)
        # cifar10 stats
...
if __name__ == "__main__":
    augmentation_config_file = "/mnt/Blade1T/src/github/data_augmentation_robustness/augmentation_dicts/benchmark/cls/cls_1.0.yaml"
    device = torch.device("cuda:0")
    train = False
    ds = AdvCIFAR10DSET(PATH_DICT['advcifar10'], train, augmentation_config_file, device, db_name='robust',
                        download=False)
